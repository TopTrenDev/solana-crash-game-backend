...
var commonValidations = {
  id: import_zod.z.string().refine((data) => !isNaN(Number(data)), "ID must be a numeric value").transform(Number).refine((num2) => num2 > 0, "ID must be a positive number")
...
var ServiceResponseSchema = (dataSchema) => import_zod3.z.object({
  success: import_zod3.z.boolean(),
  message: import_zod3.z.string(),
  responseObject: dataSchema.optional(),
  statusCode: import_zod3.z.number()
});
...
// src/api/healthCheck/healthCheckRouter.ts
var healthCheckRegistry = new import_zod_to_openapi2.OpenAPIRegistry();
...
function generateOpenAPIDocument() {
  const registry = new import_zod_to_openapi3.OpenAPIRegistry([healthCheckRegistry, userRegistry]);
  const generator = new import_zod_to_openapi3.OpenApiGeneratorV3(registry.definitions);
...
var customLogLevel = (_req, res, err) => {
  if (err || res.statusCode >= import_http_status_codes5.StatusCodes.INTERNAL_SERVER_ERROR)
    return "error" /* Error */;
  if (res.statusCode >= import_http_status_codes5.StatusCodes.BAD_REQUEST)
    return "warn" /* Warn */;
  if (res.statusCode >= import_http_status_codes5.StatusCodes.MULTIPLE_CHOICES)
    return "silent" /* Silent */;
  return "info" /* Info */;
};
var customSuccessMessage = (req, res) => {
  if (res.statusCode === import_http_status_codes5.StatusCodes.NOT_FOUND)
    return (0, import_http_status_codes5.getReasonPhrase)(import_http_status_codes5.StatusCodes.NOT_FOUND);
...
var userService = {
  ...
  findAll: async () => {
    try {
      ...
      if (!users2) {
        return new ServiceResponse(1 /* Failed */, "No Users found", null, import_http_status_codes6.StatusCodes.NOT_FOUND);
      }
      return new ServiceResponse(0 /* Success */, "Users found", users2, import_http_status_codes6.StatusCodes.OK);
    } catch (ex) {
      const errorMessage = `Error finding all users: $${ex.message}`;
      logger.error(errorMessage);
      return new ServiceResponse(1 /* Failed */, errorMessage, null, import_http_status_codes6.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  },
  ...
  findById: async (id) => {
    try {
      ...
      if (!user) {
        return new ServiceResponse(1 /* Failed */, "User not found", null, import_http_status_codes6.StatusCodes.NOT_FOUND);
      }
      return new ServiceResponse(0 /* Success */, "User found", user, import_http_status_codes6.StatusCodes.OK);
    } catch (ex) {
...
// src/api/user/userRouter.ts
var userRegistry = new import_zod_to_openapi4.OpenAPIRegistry();
