...
describe("Error Handler Middleware", () => {
  ...
  beforeAll(() => {
    ...
    app.use("*", (req, res) => res.status(import_http_status_codes2.StatusCodes.NOT_FOUND).send("Not Found"));
  });
  describe("Handling unknown routes", () => {
    it("returns 404 for unknown routes", async () => {
      const response = await (0, import_supertest.default)(app).get("/this-route-does-not-exist");
      expect(response.status).toBe(import_http_status_codes2.StatusCodes.NOT_FOUND);
    });
  });
  describe("Handling thrown errors", () => {
    it("handles thrown errors with a 500 status code", async () => {
      ...
      expect(response.status).toBe(import_http_status_codes2.StatusCodes.INTERNAL_SERVER_ERROR);
    });
  });
  describe("Handling errors passed to next()", () => {
    it("handles errors passed to next() with a 500 status code", async () => {
      const response = await (0, import_supertest.default)(app).get("/next-error");
      expect(response.status).toBe(import_http_status_codes2.StatusCodes.INTERNAL_SERVER_ERROR);
    });
  });
});
