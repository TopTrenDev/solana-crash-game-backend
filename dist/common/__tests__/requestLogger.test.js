...
var customLogLevel = (_req, res, err) => {
  if (err || res.statusCode >= import_http_status_codes2.StatusCodes.INTERNAL_SERVER_ERROR)
    return "error" /* Error */;
  if (res.statusCode >= import_http_status_codes2.StatusCodes.BAD_REQUEST)
    return "warn" /* Warn */;
  if (res.statusCode >= import_http_status_codes2.StatusCodes.MULTIPLE_CHOICES)
    return "silent" /* Silent */;
  return "info" /* Info */;
};
var customSuccessMessage = (req, res) => {
  if (res.statusCode === import_http_status_codes2.StatusCodes.NOT_FOUND)
    return (0, import_http_status_codes2.getReasonPhrase)(import_http_status_codes2.StatusCodes.NOT_FOUND);
...
describe("Request Logger Middleware", () => {
  ...
  beforeAll(() => {
    ...
    app.get("/success", (req, res) => res.status(import_http_status_codes3.StatusCodes.OK).send("Success"));
  ...
  describe("Successful requests", () => {
    it("logs successful requests", async () => {
      const response = await (0, import_supertest.default)(app).get("/success");
      expect(response.status).toBe(import_http_status_codes3.StatusCodes.OK);
    });
    it("checks existing request id", async () => {
      const requestId = "test-request-id";
      const response = await (0, import_supertest.default)(app).get("/success").set("X-Request-Id", requestId);
      expect(response.status).toBe(import_http_status_codes3.StatusCodes.OK);
    });
  });
  describe("Re-directions", () => {
    it("logs re-directions correctly", async () => {
      const response = await (0, import_supertest.default)(app).get("/redirect");
      expect(response.status).toBe(import_http_status_codes3.StatusCodes.MOVED_TEMPORARILY);
    });
  });
  describe("Error handling", () => {
    it("logs thrown errors with a 500 status code", async () => {
      const response = await (0, import_supertest.default)(app).get("/error");
      expect(response.status).toBe(import_http_status_codes3.StatusCodes.INTERNAL_SERVER_ERROR);
    });
    it("logs 404 for unknown routes", async () => {
      const response = await (0, import_supertest.default)(app).get("/unknown");
      expect(response.status).toBe(import_http_status_codes3.StatusCodes.NOT_FOUND);
    });
  });
});
