...
export class CrashGameSocketController {
  ...
  public autoCrashBetHandler = async (data: TAutoCrashBetPayload) => {
    ...
    try {
      const { betAmount, cashoutPoint, count } = data;

      if (this.user?.credit < betAmount) {
        return this.socket.emit(
  ...
  public joinGameHandler = async (
    ...
    const { target, betAmount: crashBetAmount } = data;
    ...
    try {
      ...
      if ((this.user!.credit ?? 0) < parseFloat(crashBetAmount.toFixed(2))) {
        delete CrashGameSocketController.gameStatus.pending[userId];
        CrashGameSocketController.gameStatus.pendingCount--;
        return this.socket.emit(
          "game-join-error",
      ...
      if (
        ...
        if (CrashGameSocketController.gameStatus.nextPending[userId]) {
          delete CrashGameSocketController.gameStatus.nextPending[userId];
          return this.socket.emit("next-round-join-cancel");
        } else {
          CrashGameSocketController.gameStatus.nextPending[userId] = {
            betAmount: crashBetAmount,
            autoCashOut,
            username: this.user!.username,
          };
          return this.socket.emit("next-round-join-success");
        }
      }
      ...
      const newWagerNeededForWithdrawValue =
        (this.user!.wagerNeededForWithdraw || 0) +
        Math.abs(parseFloat(crashBetAmount.toFixed(2)));
      const newPlayedNumber = this.user!.played + 1;
      const newLeaderboardValue =
        (this.user!.leaderboard?.["crash"]?.betAmount || 0) +
        Math.abs(parseFloat(crashBetAmount.toFixed(2)));
      ...
      // Update local wallet
      this.socket.emit("update-wallet", newCreditValue);
      ...
      const newBet = {
        ...
        level: getVipLevelFromWager(
          this.user!.wager ? this.user!.wager ?? 0 : 0
        ),
      ...
      if (siteUser) {
        ...
        let newSiteCrashCreditValue = 0;

        if (siteUser?.credit) {
          newSiteCreditValue = (siteUser?.credit || 0) + crashBetAmount;
        } else {
          newSiteCreditValue = crashBetAmount;
        }

        if (siteUser?.leaderboard?.["crash"]) {
          newSiteCrashCreditValue =
            (siteUser?.leaderboard?.["crash"]?.winAmount || 0) + crashBetAmount;
        } else {
          newSiteCrashCreditValue = crashBetAmount;
        }

        await this.userService.updateById(SITE_USER_ID, {
          $set: {
            [`credit`]: newSiteCreditValue,
            [`leaderboard.crash.winAmount`]: newSiteCrashCreditValue,
          },
        });
      } else {
      ...
      return this.socket.emit(
        "game-join-error",
        "There was an error while processing your bet"
      );
    }
  };
  ...
  public betCashoutHandler = async (
    ...
    // Check if bet exists
    if (!bet) {
      return this.socket.emit("bet-cashout-error", "Couldn't find your bet!");
    }
